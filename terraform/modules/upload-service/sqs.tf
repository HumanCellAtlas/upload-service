...
resource "aws_sqs_queue" "validation_queue" {
  name                      = "dcp-upload-validation-queue-${var.deployment_stage}"
//  Queue visibility timeout must be larger than (triggered lambda) function timeout
  visibility_timeout_seconds = 960
  message_retention_seconds = 86400
  redrive_policy            = "{\"deadLetterTargetArn\":\"${aws_sqs_queue.validation_deadletter_queue.arn}\",\"maxReceiveCount\":4}"

}

resource "aws_sqs_queue" "validation_deadletter_queue" {
  name                      = "dcp-upload-validation-deadletter-queue-${var.deployment_stage}"
  message_retention_seconds = 1209600
}
